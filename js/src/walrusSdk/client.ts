...
import { bls12381_min_pk_verify } from '../walrusWasm/walrus_wasm.js';
...
export class WalrusClient {
	...
	async createStorage({ size, epochs, walCoin }: StorageWithSizeOptions) {
		...
		return (tx: Transaction) => {
			...
			console.log('lol', [systemObject.id.id, encodedSize, epochs, coin] );
	...
	async writeBlob({
		...
		console.error(sliversByNode, blobId, metadata, rootHash);
		...
		const confirmations = await Promise.all(
			sliversByNode.map((slivers, nodeIndex) => {
				return this.writeEncodedBlobToNode({
					...
					console.log(failures, systemState.committee.n_shards);
